import type { Bit, BitPosition, Int8, Uint8, Int16, Uint16 } from './types'
...
export const getBit = (value: Int8 | Uint8, bit: BitPosition): Bit => {
  const mask = 1 << bit
  return (value & mask) === 0 ? 0 : 1
}
export const setBit = (value: Int8 | Uint8, bit: BitPosition): number => {
  const mask = 1 << bit
  return value | mask
}
export const resetBit = (value: Int8 | Uint8, bit: BitPosition): number => {
  const mask = ~(1 << bit)
  return value & mask 
}